...
namespace rgb_matrix
{
  namespace internal
  {
    ...
    {
    public:
      P4Outdoor80x40(const char* name, int even_vblock_offset, int odd_vblock_offset)
        : MultiplexMapperBase(name, 2),
        even_vblock_offset_(even_vblock_offset),
        odd_vblock_offset_(odd_vblock_offset) {
      }
      ...
      void MapSinglePanel(int x, int y, int* matrix_x, int* matrix_y) const
      {
        ...
        const int even_vblock_shift = (1 - vert_block_is_odd) * even_vblock_offset_;
        const int odd_vblock_shift = vert_block_is_odd * odd_vblock_offset_;

        if (!vert_block_is_odd) {
          int block_count = x / tile_width;
          int pixel_num = x % tile_width;
          x = block_count * tile_width + (tile_width - 1 - pixel_num); // Simplified reversal
        }

        *matrix_x = x + ((x + even_vblock_shift) / tile_width) * tile_width + odd_vblock_shift;
        *matrix_y = (y % tile_height) + tile_height * (y / (tile_height * 2));
      }
    ...
    */
    ...
    static MuxMapperList* CreateMultiplexMapperList()
    {
      ...
      result->push_back(new P4Outdoor80x40("L920F 10S", 0, 8));
